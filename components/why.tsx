    ...
    <section className="py-24 bg-[#f8fafc]">
      <div className="max-w-[1440px] mx-auto px-6">
        ...
        <div className="flex flex-col items-center mb-20 text-center">
          <div className="inline-flex items-center bg-[#1a365d]/5 rounded-full p-1 border border-[#1a365d]/10 mb-6">
            <span className="bg-[#1a365d] px-8 py-2 rounded-full text-white text-[10px] font-[1000] tracking-[0.25em] uppercase shadow-lg">
              The Advantage
            </span>
          </div>
          <h2 className="text-3xl md:text-5xl font-[1000] text-[#1a365d] tracking-tighter leading-none">
            Why the Modern Thinker <br />
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          {advantages.map((item, i) => (
            <motion.div
              ...
              className="flex flex-col sm:flex-row bg-white rounded-[40px] overflow-hidden border border-gray-100 shadow-[0_15px_45px_rgba(0,0,0,0.03)] hover:shadow-2xl transition-all duration-500 group"
            >
              {/* IMAGE SECTION (LEFT HALF) */}
              <div className="w-full sm:w-[42%] relative min-h-[300px]">
                <img 
                  src={item.image} 
                  alt={item.title} 
                ...
                {/* FLOATING ID BADGE */}
                <div className="absolute top-6 left-6 px-4 py-2 bg-white/90 backdrop-blur-md rounded-full shadow-lg border border-white/20 flex items-center gap-2">
                  <span className="text-[#1a365d] p-1 bg-[#1a365d]/5 rounded-full">{item.icon}</span>
                  <span className="text-[#1a365d] text-[9px] font-[1000] tracking-widest">{item.id}</span>
                </div>
              </div>

              {/* CONTENT SECTION (RIGHT HALF) */}
              <div className="w-full sm:w-[58%] p-8 sm:p-10 flex flex-col justify-between bg-white">
                <div className="space-y-4">
                  <div>
                    <span className="text-sky-500 text-[10px] font-black tracking-widest uppercase mb-1 block">Through {item.program}</span>
                    <h3 className="text-2xl sm:text-3xl font-[1000] text-[#1a365d] tracking-tight">{item.title}</h3>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-3 pt-2">
                    {item.benefits.map((benefit, idx) => (
                      <div key={idx} className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-50 border border-slate-100">
                        ...
                        <span className="text-[#1a365d] text-[11px] font-bold tracking-tight">{benefit}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="mt-10 pt-6 border-t border-slate-50">
                  <div className="flex justify-between items-end mb-2.5">
                    <span className="text-[10px] font-black text-[#1a365d] uppercase tracking-wider">{item.metricLabel}</span>
        ...
        <div className="mt-16 flex justify-center">
          <button className="flex items-center gap-3 text-[#1a365d] font-black uppercase text-xs tracking-[0.25em] border-b-2 border-sky-500 pb-1 transition-all hover:text-sky-500 hover:gap-5">
            Discover the method
            <ArrowRight size={16} />
          </button>
        </div>
